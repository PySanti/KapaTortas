          ...
          {extraList.map((product, index) => (
            <div
              ...
              className={classNames(
                selected.includes(product) ? "ring-2 ring-primary bg-primary-light bg-opacity-20" : "",
                "relative block cursor-pointer rounded-lg border border-gray-300 p-4 focus:outline-none"
              )}
              onClick={() => handleSizeChange(product)}
            >
              { product?.presentacion && product.presentacion.length > 0 ?   <label
                              className="text-base font-medium text-terciary"
                            >
                              ${ product.presentacion[0].precio }
                            </label> : "" }
          
              <p className="mt-1 text-sm text-terciary">
                {product.descripcion}
              </p>
              <div
                className={classNames(
