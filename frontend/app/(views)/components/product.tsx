...
export default function Product({ product }: { product: Producto }) {
    ...
    return (
        <div className="bg-secondary-light">
            <div className="mx-auto max-w-2xl px-8 py-16 pt-6 sm:px-6 sm:py-24 sm:pt-6 lg:grid lg:max-w-7xl lg:grid-cols-2 lg:gap-x-8 lg:px-8">
                ...
                <div className="lg:max-w-lg lg:self-end">
                    ...
                    <div className="mt-4">
                        <h1 className="text-4xl lg:text-5xl text-primary">{ product.titulo }</h1>
                    </div>

                    <section aria-labelledby="informacion" className="mt-4">
                        <h2 className="sr-only">Información del Producto</h2>
                        <p className="text-3xl text-terciary tracking-tight">${product.precio}</p>

                        <div className="mt-4 space-y-6">
                            <p className="text-terciary text-base">{product.descripcion.caracteristicas}</p>
                        </div>
                ...
                <div className="mt-10 lg:col-start-2 lg:row-span-2 lg:mt-0 lg:self-center">
                    <div className="aspect-h-1 aspect-w-1 overflow-hidden rounded-lg">
                        {/* <Image */}
                        <ProductImage path={product.imagenes && product.imagenes.length > 0 ? product.imagenes[0] : "/images/Torta-Chocolate.png"}
                        alt={ product.titulo } />
                    </div>
                </div>
                        ...
                        <form>
                            <div className="sm:flex sm:justify-between">
                                <RadioGroup value={size} onChange={setSize}>
                                    <RadioGroup.Label 
                                        className="block text-sm font-medium">Size
                                        </RadioGroup.Label>
                                        <div className="mt-1 grid grid-cols-1 gap-4 items-center sm:grid-cols-2">
                                            { product.proporcion.map((size, index) => (
                                                ...
                                                >
                                                    {({ active, checked }) => (
                                                        <>
                                                            <RadioGroup.Label as="p" className="text-base font-medium text-terciary">
                                                                { size }
                                                            </RadioGroup.Label>
                                                            <RadioGroup.Description as="p" className="mt-1 text-sm text-terciary">
                            ...
                            <div className="mt-10 text-center">
                                <Button type="button" className="text-center text-base py-7 rounded-full">
                                    Agregar al Carrito
                                </Button>
                            </div>
